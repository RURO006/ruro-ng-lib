...
module.exports = class DB {
     ...
     */
    constructor(poolOption = null, connectionOption = null, queryTimeout = 40000) {
     ...
     */
    async oneQuery(commandText = '', values = [], connection = null, destroy = true) {
        console.log(commandText);
        console.log(values);
        if (!connection) connection = this.createConn();
        if (connection.hasBeginTrans) throw new Error('connection有包含交易，請改用queryTrans來執行。');
        return new Promise((resolve, reject) => {
            connection.query(
                {
                    sql: commandText,
    ...
    async batchQuery(commandArray = [], usePool = false, useTrans = false) {
        let results = [];
        let conn = null;
        if (!(commandArray instanceof Array) && 'commandText' in commandArray && 'value' in commandArray)
        ...
        if (useTrans) {
            ...
            try {
                for (let i = 0; i < commandArray.length; i++) {
                    ...
                    oneResult = await this.queryTrans(conn, commandArray[i].commandText, value);
            ...
            for (let i = 0; i < commandArray.length; i++) {
                ...
                oneResult = await this.oneQuery(commandArray[i].commandText, value, conn, i == commandArray.length - 1);
                // 如果有設定resultName，則新增resultName當回傳參考。
                if (commandArray[i].resultName) {
                    results[commandArray[i].resultName] = oneResult;
                }
