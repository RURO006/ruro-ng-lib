export class ImgOrientationRotate {
    af2Base64(af: any) {
        const str: any = [].reduce.call(new Uint8Array(af), (p, c) => p + String.fromCharCode(c), '');
    ...
    rotateCtx(orieVal: number, ctx: CanvasRenderingContext2D, width: number, height: number) {
        const qq = new HTMLCanvasElement();
        switch (orieVal) {
            case 2:
    ...
    async fixImg(files: File): Promise<string> {
